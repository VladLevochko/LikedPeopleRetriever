...
public class BlazeFace extends MyFaceDetector {
    ...
    public List<Rect> detectFaces(Bitmap bitmap) {
        float sx = (float) INPUT_SIZE_WIDTH / bitmap.getWidth();
        float sy = (float) INPUT_SIZE_HEIGHT / bitmap.getHeight();
        Matrix m = new Matrix();
        m.postScale(sx, sy);
        Bitmap resized = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), m, false);

        List<RectF> faces = detect(resized);
        return faces.stream()
                .map(rect -> new Rect(
    ...
    public synchronized List<RectF> detect(Bitmap bitmap) {
